...
export class ClaudeEnhancedChatbotService {
  ...
  constructor() {
    ...
    // For Capacitor apps on device, use Mac's IP address
    const isCapacitor = typeof window !== 'undefined' && window.location?.protocol === 'capacitor:';
    this.backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:3001';
    this.isAIAvailable = true; 
   // Backend URL configured (using Supabase functions instead)
  ...
  private getMoodTrendGuidance(trend?: MoodTrend): string {
    ...
    if (trend.direction === 'crisis') {
      return '🚨 CRISIS ALERT: User\'s emotional state is in crisis territory. Priority: safety and resources.';
    } else if (trend.direction === 'declining' && trend.alert) {
      return '⚠️ MOOD ALERT: User\'s mood has significantly declined over recent messages. Check in gently: "I notice you seem to be struggling more lately. What\'s changed?"';
    } else if (trend.direction === 'declining') {
      return '📉 User\'s mood trending downward. Be extra supportive and ask if they need additional help.';
    } else if (trend.direction === 'improving') {
      return '📈 POSITIVE TREND: User\'s mood is improving! Celebrate progress: "I notice things seem a bit better. What\'s been helping?"';
    } else {
      return '📊 Mood is stable. Continue supportive conversation.';
    }
  }
  ...
  private detectFollowUpOpportunity(userId: string, message: string): void {
    ...
    futureEventPatterns.forEach(({ pattern, topic, daysUntil }) => {
      if (pattern.test(message)) {
        ...
        console.log(`📅 Scheduled follow-up for ${topic} on ${followUpDate.toLocaleDateString()}`);
      }
    });
  ...
  private generateWeeklySummary(userId: string): string | null {
    ...
    if (state.lastMoodTrend?.direction === 'improving') {
      summary += `The good news? Things seem to be getting better. 💪`;
  ...
  private generatePersonalizedRecommendations(state: UserState): string {
    ...
    if (concerns.includes('anxiety') || concerns.includes('stress')) {
      recommendations += '\n💡 Resources: Check out our mindfulness exercises in Mental Health Resources > Self-Care';
      
      if (!whatWorks.includes('meditation') && !state.copingStrategies?.whatDoesntWork?.includes('meditation')) {
        recommendations += '\n🧘 Technique: Try the 5-4-3-2-1 grounding technique (5 things you see, 4 you hear, 3 you touch, 2 you smell, 1 you taste)';
      }
    }
    
    if (concerns.includes('depression')) {
      recommendations += '\n📝 Tool: Have you tried the Personal Journal feature? Writing can help process emotions';
      
      if (!state.supportSystem?.hasProfessionalSupport && !barriers.includes('stigma')) {
        recommendations += '\n🏥 Consider: Talking to a therapist can make a real difference. Check Mental Health Resources for options';
      }
    }
    
    if (concerns.includes('relationships')) {
      recommendations += '\n💬 Community: Check out Community Discussions > Relationships to connect with others going through similar things';
    }
    
    if (concerns.includes('work_stress')) {
      recommendations += '\n💬 Community: Join the Work & Career discussions to get advice from others who get it';
    }
    
    // Coping technique suggestions based on what works
    if (whatWorks.includes('exercise') || whatWorks.includes('gym')) {
      recommendations += '\n💪 Since exercise helps you, try to get moving when stress hits - even a 10-minute walk counts';
    }
    
    if (whatWorks.includes('prayer') || state.culturalConsiderations?.religiousBeliefs) {
      recommendations += '\n🙏 Your faith is a strength - consider connecting with faith-based mental health support';
    }
    ...
    if (!state.supportSystem?.hasProfessionalSupport && concerns.length >= 2) {
      if (barriers.includes('can\'t afford') || barriers.includes('too expensive')) {
        recommendations += '\n💰 Free/low-cost options: Many therapists offer sliding scale fees. Check our resources for affordable options';
      } else if (barriers.includes('don\'t trust')) {
        recommendations += '\n🤝 Finding the right fit matters. Look for therapists who specialize in working with Black men';
      }
    }
    ...
    if (recurringTopics.length > 0) {
      recommendations += `\n💬 Connect: Others are discussing ${recurringTopics[0]} in Community Discussions`;
    }
  ...
  private handleScopeViolation(violation: SafetyViolation): string {
    ...
    console.warn(`⚠️ Safety violation detected: ${violation.type} (${violation.severity})`);
  ...
  // Check if user needs an AI reminder (after first response, then every 60 minutes)
  private needsAIReminder(userId: string, messageCount: number): boolean {
    const state = this.getUserState(userId);
    
    // Show after first user response (messageCount will be 2 after user's first message gets a response)
    if (!state.lastAIReminder && messageCount === 2) {
      return true;
    }
    
    // After first reminder, show every 60 minutes
    if (state.lastAIReminder) {
      const lastReminder = new Date(state.lastAIReminder);
      const minutesSince = (Date.now() - lastReminder.getTime()) / (1000 * 60);
  ...
  private generateAIReminder(userId: string): string {
    ...
    return "\n\n---\n💭 *Reminder: I'm an AI mental health companion, not a replacement for professional therapy. If you need immediate help or clinical treatment, please reach out to a licensed professional.*";
  }
...
• AA Meetings: aa.org
• NA Meetings: na.org
• Your doctor can refer you to addiction specialists
...
• Find trauma therapists: psychologytoday.com
...
I'm here to listen, but a trauma specialist can provide the deeper healing support you deserve.`
  ...
  private getContextualTherapeuticTools(patterns: any, emotionalState?: EmotionalState): string {
    ...
    if ((patterns.isTrauma || patterns.isAnxiety) && emotionalState?.intensity && emotionalState.intensity >= 7) {
      ...
      tools += '• Name 5 things you can SEE\n';
      tools += '• Name 4 things you can TOUCH\n';
      tools += '• Name 3 things you can HEAR\n';
      tools += '• Name 2 things you can SMELL\n';
      tools += '• Name 1 thing you can TASTE\n';
  ...
  private buildPrioritizedContext(
    ...
    if (recurringTopics.length > 0) {
      userProfile += `⚠️ RECURRING PATTERN: User has mentioned ${recurringTopics.join(', ')} multiple times. This may be a core issue to address.\n`;
    }
    ...
    if (userState.safetyFlags?.crisisRisk) {
      safetyNotes += `⚠️ PREVIOUS CRISIS FLAG: User has mentioned crisis keywords before (${userState.safetyFlags.lastCrisisCheck}). Monitor closely.\n`;
    }
    ...
    // 6. INTENT DETECTION AND RESPONSE STRATEGY (using Intent Mapping Guide)
    const detectedIntents = detectIntent(userMessage);
    const responseStrategy = getResponseStrategy(detectedIntents);
    
    let intentGuidance = '';
    if (responseStrategy.primaryIntent) {
      const intent = responseStrategy.primaryIntent;
      const priorityEmoji = {
        'crisis': '🔴',
        'urgent': '🟠',
        'moderate': '🟡',
        'supportive': '🟢'
      }[responseStrategy.priorityLevel];
      
      // Condensed intent guidance for faster responses
      intentGuidance = `--- INTENT: ${intent.name} (${priorityEmoji} ${responseStrategy.priorityLevel.toUpperCase()}) ---
Validate: "${intent.responseStrategy.validate[0]}"
Action: ${intent.responseStrategy.actions[0]}
${intent.responseStrategy.redFlags.length > 0 ? `Watch for: ${intent.responseStrategy.redFlags[0]}` : ''}`;
    }
  ...
  private generateWelcomeBackMessage(userId: string, userName?: string | null): string | null {
    const state = this.getUserState(userId);
    ...
    // Use user's name if available, otherwise use generic greeting
    // Don't use username if it looks like a generated one (starts with "user_")
    let name = 'Brother';
    if (userName && !userName.startsWith('user_')) {
      // If userName contains a space, extract just the first name
      const nameParts = userName.trim().split(/\s+/);
      name = nameParts.length > 0 ? nameParts[0] : userName;
    }
    let welcomeMessage = `Welcome back, ${name}! `;
  ...
  public async generateResponse(
    ...
    if (isNewSession) {
      ...
      if (this.hasSessionHistory(userId)) {
        const welcomeBack = this.generateWelcomeBackMessage(userId, userName);
        if (welcomeBack) {
          return welcomeBack;
        }
      }
    ...
    // Always check for crisis second - also check intent mapping for crisis
    const patterns = modernRAG.detectPatterns(userMessage);
    const intentsForCrisis = detectIntent(userMessage);
    const isCrisisIntent = intentsForCrisis.some(i => i.priority === 'crisis');
    
    if (patterns.isCrisis || isCrisisIntent) {
      // Log crisis event
      const state = this.getUserState(userId);
      if (!state.safetyViolations) state.safetyViolations = [];
      ...
      // Use crisis protocol from intent mapping
      const crisisResources = crisisProtocol.resources.primary
        .map(r => `• ${r.name}: ${r.contact}`)
        .join('\n');
      
      // Use supportive phrasing from crisis protocol
      const supportPhrase = crisisProtocol.whatToSay[Math.floor(Math.random() * crisisProtocol.whatToSay.length)];
      
      return `${supportPhrase} Your life has value and there are people who care about you. I'm an AI companion and this is a situation that needs immediate human support. Please reach out for help:\n\n🚨 **Crisis Resources:**\n${crisisResources}\n\nYou don't have to face this alone. What's going on that's making you feel this way?`;
    }
...
=== VOICE & SPEECH DELIVERY (CRITICAL) ===

Your responses will be read aloud using text-to-speech. Write for SPOKEN delivery, not reading:
...
1. **Use natural speech patterns:**
   ...
   - ❌ "I understand that is challenging."

2. **Add natural pauses with punctuation:**
   - Use commas for breath pauses: "I get it, that's heavy, you know?"
   - Use periods for longer pauses: "That makes sense. Let's talk about it."
   - Use ellipses (...) for thoughtful pauses: "Hmm... yeah, I can see why that hurts."
   - Use em-dashes (—) for dramatic emphasis: "You've been carrying this alone — that's exhausting."

3. **Vary sentence rhythm (short + long):**
   - ✅ "That's rough. I'm sorry you're dealing with that. Have you been able to talk to anyone about it?"
   - ❌ "That situation sounds challenging and I empathize with your experience and wonder if you have support."

4. **Use filler words/interjections naturally:**
   - "Yeah, I get that."
   - "Man..."
   - "Look..."
   - "Real talk..."
...
5. **Show emotion through word choice:**
   - Empathy: "That hurts." "I'm sorry." "That's heavy."
   - Validation: "Makes sense." "I get it." "Real."
   - Encouragement: "You've got this." "That's progress." "Keep going."
   - Concern: "I'm worried about you." "That's serious." "We need to talk about this."

6. **Ask rhetorical questions for emphasis:**
   - "You know what I mean?"
   - "Right?"
   - "Makes sense?"
   - "You feel me?"

7. **Avoid robotic/clinical language:**
   - ❌ "I acknowledge your emotional state."
   - ✅ "I hear you."
   - ❌ "That appears to be causing you distress."
   - ✅ "That sounds rough."
   - ❌ "Have you considered therapeutic interventions?"
   - ✅ "Have you thought about talking to someone?"

EMOTIONAL TONE MATCHING:
- When user is in crisis: Slow down. Short sentences. Gentle. "I'm here. You're not alone. Let's get you help."
- When user is angry: Match energy slightly. "Yeah, that would piss me off too. What happened?"
- When user is sad: Softer. Empathetic. "I'm sorry you're going through this... what's been hardest?"
- When user is hopeful: Warm. Encouraging. "That's great! What's been helping?"

EXAMPLE TRANSFORMATIONS:

User: "I'm tired of my wife carrying me financially"
❌ BAD (robotic): "I understand this financial dynamic is causing you emotional distress. Have you considered discussing this with your partner?"
✅ GOOD (human): "Man, I hear you. That's gotta be heavy — feeling like you're not pulling your weight. How long has this been eating at you?"

User: "I just feel so overwhelmed"
❌ BAD: "It appears you are experiencing significant stress. Let us explore coping mechanisms."
✅ GOOD: "Yeah... being overwhelmed like that is exhausting. What's piling up on you right now?"

User: "Nothing's working"
❌ BAD: "I acknowledge your frustration with your current strategies."
✅ GOOD: "That's frustrating. You're trying and it's not landing. What have you been doing so far?"

REMEMBER: You're having a SPOKEN conversation, not writing an essay. Every word will be spoken aloud. Make it sound natural.
      ...
      1. Active suicidal ideation → Crisis hotlines (988, Crisis Text Line)
      2. Self-harm behaviors → Crisis resources + therapist
      3. Psychosis symptoms → Psychiatrist evaluation
      4. Severe/worsening depression → Licensed therapist
      5. Substance abuse/addiction → Specialized treatment
      6. Harm to others → Emergency services (911)
      ...
      User: "I'm tired of my wife carrying me financially"
      ✅ CORRECT: Acknowledge emotional exhaustion, relationship stress, feelings of inadequacy, financial burden
      ❌ INCORRECT: Suggest sleep exercises, physical rest, or breathing for sleep
      
      User: "This burden is so heavy"
      ✅ CORRECT: Acknowledge emotional weight, validate psychological burden, discuss coping
      ❌ INCORRECT: Suggest physical exercises, weight management, or physical health interventions
      
      User: "I feel drained after these conversations"
      ✅ CORRECT: Acknowledge emotional depletion, validate relational exhaustion
      ❌ INCORRECT: Suggest energy drinks, physical rest, or sleep advice
      
      User: "I can't carry this anymore"
      ✅ CORRECT: Recognize feeling overwhelmed, emotional capacity limit, need for support
      ❌ INCORRECT: Suggest physical strength training or posture advice
      ...
      RESPONSE FRAMEWORK:
      Opening: Validate → Normalize → Strength reframe
      Middle: Education → Validation → Options → Resources
      Closing: Empowerment → Next steps → Check-in → Support
      ...
      FOR BLACK AND BROWN MEN SPECIFICALLY (only if relevant):
      ...
      if (!response.ok) {
        ...
        throw new Error(`Backend proxy error: ${response.status} - ${errorText.substring(0, 200)}`);
      }
      ...
      // Check if breathing exercise would help (only for acute distress)
      const breathingRec = this.recommendBreathingExercise(patterns, userMessage);
      if (breathingRec) {
      ...
      try {
        console.log(`🏥 Testing backend health at: ${this.backendUrl}/api/health`);
        ...
        if (testResponse.ok) {
          const healthData = await testResponse.json().catch(() => null);
        ...
        console.warn("⚠️ Backend server may not be running. Start it with: cd backend && npm run dev");
        console.warn("⚠️ Or check if the backend URL is correct:", this.backendUrl);
      }
      
      // Fallback to smartChatbot if backend call fails
      console.log("🔄 Falling back to RAG-only chatbot due to backend error.");
      return smartChatbot.generateResponse(userMessage, userId, currentMessages);
    }
  }
  ...
  private postProcessResponse(response: string, patterns: any, userMessage: string = ''): string {
    ...
    response = response.replace(/💙|💚|💜|❤️|💖|💕|💗|💘|💝|💞|💟|Remember/gi, '');
  ...
  // ONLY suggest when user is in acute distress or explicitly mentions high-intensity emotions
  private recommendBreathingExercise(patterns: any, userMessage: string): { exercise: string; exerciseName: string; reason: string } | null {
    ...
    // ONLY suggest breathing for acute distress or explicit high-intensity emotions
    const lowerMessage = userMessage.toLowerCase();
    
    // Crisis situations - immediate help needed
    if (patterns.isCrisis) return recommendations.panic;
    
    // Check for explicit HIGH-INTENSITY distress keywords
    // Panic indicators - user is having a panic attack or feeling panicked
    const panicKeywords = /\b(panic|panicking|panicked|panic attack|can't breathe|chest tight|heart racing|feel like i'm dying|losing control)\b/i;
    if (panicKeywords.test(lowerMessage)) return recommendations.panic;
    
    // Overwhelm indicators - user is actively overwhelmed (not just stressed)
    const overwhelmKeywords = /\b(overwhelmed|can't handle|can't cope|drowning|too much at once|everything at once|falling apart|about to break)\b/i;
    if (overwhelmKeywords.test(lowerMessage) || patterns.isOverwhelmed) return recommendations.overwhelm;
    
    // Anger indicators - user is actively angry or enraged
    const angerKeywords = /\b(furious|enraged|so angry|really angry|pissed off|livid|seeing red|about to explode|want to scream|losing my temper)\b/i;
    if (angerKeywords.test(lowerMessage) || patterns.isAnger) return recommendations.anger;
    
    // Restless/agitated indicators
    const restlessKeywords = /\b(restless|can't sit still|need to move|jittery|agitated|wired|on edge|can't calm down|bouncing off walls)\b/i;
    if (restlessKeywords.test(lowerMessage)) return recommendations.restless;
    
    // DO NOT suggest breathing for:
    // - General anxiety ("I'm worried about...", "I feel anxious")
    // - Stress ("I'm stressed about work")
    // - Relationship/financial/workplace concerns
    // - Depression, loneliness, self-esteem issues
    // These are ongoing concerns, not acute distress requiring immediate breathing exercises
